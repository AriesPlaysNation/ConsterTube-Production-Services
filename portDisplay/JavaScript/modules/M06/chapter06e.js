...
}// END FUNCTION removeSelectDefaults
...
function validateAddress(fieldsetId) {
   ...
   try {
      ...
      }// END currentElement.selectedIndex
   ...
   try {
      for (var i = 0; i < elementCount; i++) {
         ...
         if (currentElement.selectedIndex === -1) {
            ...
            currentElement.style.border = "";
         }// END IF currentElement.selectedElements
      }// END FOR
      if (fieldsetValidity === false) {
...
}// END FUNCTION validateDeliveryDate
   ...
   try { 
      ...
      for (var i = 0; i < elementCount; i++) {
         ...
         if (currentElement.selectedIndex === -1) {
            ...
            currentElement.style.border = "";
         }// END IF currentElement.selectedIndex
      }// END FOR
...
function validateCreateAccount() {
   ...
   try {
      ...
      } // END IF NOT ALL FIELDS ARE BLANK
   } // END TRY
   catch(msg) {
      errorDiv.innerHTML = msg;
      errorDiv.style.display = "block";
      ...
      } // END IF passwordMismatch
      formValidity = false;
   }// END CATCH
}// END FUNCTION validateCreateAccount
...
function validateForm(evt) {
   ...
   }// END IF evt.preventDefault
   formValidity = true; // reset value for revalidation
   validateAddress("billingAddress"); 	// line 127
   validateAddress("deliveryAddress");	// line 127 
   validateDeliveryDate();				// line 172
   validatePayment();					// line 203
   validateMessage();					// line 266
   validateCreateAccount();				// line 287
   validateNumbers();					// line 336
...
function createEventListeners() {
   ...
   if (deliveryMonth.addEventListener) {
     deliveryMonth.addEventListener("change", updateDays, false); // line 31
   } else if (deliveryMonth.attachEvent)  {
     deliveryMonth.attachEvent("onchange", updateDays);
   }// END IF deliveryMonth
   ...
   if (same.addEventListener) {
     same.addEventListener("click", copyBillingAddress, false); // line 67
   } else if (same.attachEvent)  {
     same.attachEvent("onclick", copyBillingAddress);
   }// END IF same.addEventListner

   var messageBox = document.getElementById("customText");
   if (messageBox.addEventListener) {
     messageBox.addEventListener("blur", autocheckCustom, false); // line 84
   } else if (messageBox.attachEvent)  {
     messageBox.attachEvent("onblur", autocheckCustom); 
   }// END IF messageBox.addEventListener

   var form = document.getElementsByTagName("form")[0];
   if (form.addEventListener) {
      form.addEventListener("submit", validateForm, false); // line 376
   } else if (form.attachEvent) {
      form.attachEvent("onsubmit", validateForm);
   }// END IF form.addEventListener
}// END FUNCTION createEventListeners

/* run initial form configuration functions */
function setUpPage() {
   removeSelectDefaults();		// line 58
   setupDays();					// line 11
   createEventListeners();		// line 402
   generatePlaceholder();		// line 111
}// END FUNCTION setUpPage

/* run setup functions when page finishes loading */
if (window.addEventListener) {
   window.addEventListener("load", setUpPage, false); // line 440
